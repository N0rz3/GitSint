import requests, json, random, sys, os, time, trio, argparse
...
[?] {data['name'].replace("e", "3").replace("i", "1").replace("E", "3").replace("I", "1").replace(" ", "").replace("A", "4").replace("a", "4").replace("b", "8").replace("B", "8")+"@"+random.choice(utils.domain_list)} -> @{data['name']}
  """)
      ...
      choice = input(utils.Reset+"\n[?] Do you want to export in json format (y/n): ")
      if choice == "y":
        file = input("[+] Enter your reports file : ")
        database = f"""
            "login": "{data['login']}",
            "name": "{data['name']}",
            "id": "{data['id']}",
      ...
      """
        ...
        with open(file, "w+", encoding="utf-8")as file:
            ...
            file.close()
            print(utils.Light_Green+"[+] Export finish !")
      else:
          print()
      sys.exit()
